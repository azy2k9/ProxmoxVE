...
default_setup() {
  ...
  pct exec "$CTID" -- /bin/bash -c "source <(wget -qLO - https://raw.githubusercontent.com/azy2k9/ProxmoxVE/remz/misc/install.func) && color && verb_ip6 && catch_errors && setting_up_container && network_check && update_os" || exit
...
if [[ "${SHARED_MOUNT}" == "yes" ]]; then
  ...
  if user_exists "$SHARE_USER"; then
    ...
    echo "mp0: cctv:subvol-102-disk-1,mp=/media/frigate,size=6000G" >> /etc/pve/lxc/${CTID}.conf
...
if [[ "${NVIDIA_PASSTHROUGH}" == "yes" ]]; then
  ...
  source <(curl -s https://raw.githubusercontent.com/azy2k9/ProxmoxVE/remz/misc/nvidia.func)
